...
echo "        starts automatically (no extra click) -> header avatar updates."
echo "      - 'Remove avatar' (shown only when an avatar is uploaded) -> PUT /auth/me"
echo "        { avatar: null } -> avatar falls back to /placeholder-avatar.svg and the"
echo "        primary button returns to 'Upload image'."
